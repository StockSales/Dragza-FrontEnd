...
const EditActiveIngredient = () => {
  ...
  const {updateActiveIngredient, loading: updatingProductById} = useUpdateActiveIngredient()
  ...
  const handleUpdateProduct = async (productId: string, formData: ActiveIngredient) => {
    if (!formData.name) {
    ...
    try {
      const {success, error} = await updateActiveIngredient(productId, formData)
       if (success) {
  ...
  return (
    <div className=" grid grid-cols-12  gap-4  rounded-lg">
      ...
      <div className="col-span-12 flex justify-end">
        <Button onClick={() => handleUpdateProduct(activeIngredientId, formData)}>{updatingProductById ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : null}Update Active Ingredients</Button>
      </div>
    </div>
  );
};
